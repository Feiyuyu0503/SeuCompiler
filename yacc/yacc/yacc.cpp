		...
		while (1)
		{
			...
			if (strcmp(seg, "|") == 0 || strcmp(seg, ";") == 0)
			{
				if (ptr > 0)
				{
					...
					//2021.1.5
					cout << temp.ifsubp << "  " << temp.subp << endl;
			...
			else if(seg[0]=='{')
			{
				...
				string a;
				a = seg;
				temp.subp = a.substr(1, a.length() - 2);
				line++;
				input.getline(getchar, BUFSIZ);
				seg = strtok(getchar, "\t ");
			}
				...
				if (isNonterminal(tempItem.rightp[dot], nonterminal))//如果圆点后符号为非终结符，则进行等价项扩展
				{
					...
					for (it = Item.begin(); it != Item.end(); it++)
					{
						if ((*it).leftp == tempItem.rightp[dot])//如果某个产生式的左部为当前LR1项待约非终结符
						{
							...
							//2021.1.5						
							toolItem.ifsubp=(*it).ifsubp;
							toolItem.subp=(*it).subp;
		...
		for (edge = tag.begin(); edge != tag.end(); edge++)
		{
			...
			for (vitem = temp.items.begin(); vitem != temp.items.end(); vitem++)
			{
				if ((*edge) == (*vitem).rightp[(*vitem).dot])//如果边上的符号等于某LR1项圆点后的符号
				{
                    ...
                    //2021.1.5
					yaitem.ifsubp=(*vitem).ifsubp;
					yaitem.subp = (*vitem).subp;
...
void gencode(table* mtable)
{
	...
	gc("};", 0);
	//2021 1.5
	gc("vector<quat> quattable(200);", 0);
	gc("quat quatdeal(string subp)", 0);
	gc("{", 0);
	gc("if (subp.find(\"new\") != string::npos)", 1);
	gc("{", 1);
	gc("bool ifnewtemp = 1;", 2);
	gc("char* a = (char*)subp.data();", 2);
	gc("char* temp = strtok(a,\"\\t \");", 2);
	gc("temp = strtok(NULL, \",\");", 2);
	gc("string op = temp;", 2);
	gc("temp = strtok(NULL, \",\");", 2);
	gc("string op1 = temp;", 2);
	gc("temp = strtok(NULL, \",\");", 2);
	gc("string op2 = temp;", 2);
	gc("temp = strtok(NULL, \";\");", 2);
	gc("string res = temp;", 2);
	gc("int num;", 2);
	gc("if (op1 == \"@\" && op2 == \"@\")", 2);
	gc("num = 0;", 3);
	gc("else if (op1 == \"@\" || op2 == \"@\")", 2);
	gc("num = 1;", 3);
	gc("else", 2);
	gc("num = 2;", 3);
	gc("quat qgf;", 2);
	gc("qgf.op = op;qgf.op1 = op1;qgf.op2 = op2;qgf.res = res;qgf.num = num;qgf.ifnewtemp = ifnewtemp;", 2);
	gc("return qgf;}", 2);
	gc("else {", 1);
	gc("bool ifnewtemp = 0;", 2);
	gc("char* a = (char*)subp.data();", 2);
	gc("char* temp = strtok(a, \",\");", 2);
	gc("string op = temp;", 2);
	gc("temp = strtok(NULL, \",\");", 2);
	...
	gc("void tableAnalyse(stack<token>& mtokenstack, stack<int>& mnstack,stack<string>& subpstack, vector<string>& moutbuffer, token t, table2*& GOTO2)", 0);
	gc("{", 0);
	...
	gc("if (productions[num].rightp[0] != \"epsilon\")", 5);
	gc("{", 5);
	...
	gc("tableAnalyse(mtokenstack, mnstack,subpstack, moutbuffer, a, GOTO2);", 4);
	gc("break;", 4);
	gc("}", 3);
	gc("else", 3);
	gc("{", 3);
	...
	gc("}", 3);
	gc("}", 2);
	gc("if (GOTO2[mnstack.top()].tg[i] == \"epsilon\")", 2);
	gc("isepsilon = true;", 3);
	gc("if (i == GOTO2[mnstack.top()].tg.size() - 1)", 2);
	gc("{", 2);
	gc("if (isepsilon == true)", 3);
	gc("{", 3);
	gc("token b{ \"epsilon\",\"epsilon\" };", 4);
	gc("isepsilon = false;", 4);
	gc("tableAnalyse(mtokenstack, mnstack,subpstack, moutbuffer, b, GOTO2);", 4);
	gc("isused = false;", 4);
	gc("break;", 4);
	gc("}", 3);
	gc("cout << endl;", 3);
	...
	gc("}", 2);
	gc("}", 1);
	gc("isepsilon = false;", 1);
	gc("}", 0);

	gc("int main()", 0);
	gc("{", 0);
	outc << "map<int, string> I2S;\n I2S[1]=\"ASSIGN\";I2S[2]=\"COMMA\";I2S[3]=\"DIVIDE\";I2S[4]=\"DOT\";I2S[5]=\"ELSE\";I2S[6]=\"EQUAL\";I2S[7]=\"FLOAT\";I2S[8]=\"IF\";I2S[9]=\"INT\";I2S[10]=\"LBRACE\";I2S[11]=\"LBRACE\";I2S[12]=\"LPAR\";I2S[13]=\"MINUS\";I2S[14]=\"NAME\";I2S[15]=\"NUMBER\";I2S[16]=\"PLUS\";I2S[17]=\"RBRACE\";I2S[18]=\"RBRACE\";I2S[19]=\"RETURN\";I2S[20]=\"RPAR\";I2S[21]=\"SEMICOLON\";I2S[22]=\"STRUCT\";I2S[23]=\"TIMES\";" << endl;
	gc("fstream input;", 1);
	gc("char content[BUFF];", 1);
	gc("input.open(\"token.txt\", ios::in);", 1);
	gc("token nowtoken;", 2);
	...
	for (pit = Item.begin(); pit != Item.end(); pit++)
	{
		...
		outc << "\t\tproductions[" << i << "].ifsubp=" << (*pit).ifsubp << ";" << endl;//2021 1.5
		outc << "\t\tproductions[" << i << "].subp=" << "\""<<(*pit).subp <<"\""<< ";" << endl;
	...
	//2021 1.6
	gc("stack<string> subpstack;//语义栈", 2);
    gc("subpstack.push(\"#\");", 2);
	...
	gc("tableAnalyse(tokenstack, nstack, subpstack,outbuffer, now, GOTO2);", 3);
	gc("if (isused)", 3);
	gc("{", 3);
	gc("tokenqueue.pop();", 4);
...
string genQuat(string op,string arg1,string arg2,string res)
{
